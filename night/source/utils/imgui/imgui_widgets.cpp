...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec4 top = ImVec4(41 / 255.f, 41 / 255.f, 49 / 255.f, 1.0f);
    ImVec4 bottom = ImVec4(32 / 255.f, 32 / 255.f, 39 / 255.f, 1.0f);

    if (hovered)
    {
        top = Lighter(top, 1.2f);
        bottom = Lighter(bottom, 1.2f);
    }
    if (held && hovered)
    {
        top = Lighter(top, 1.1f);
        bottom = Lighter(bottom, 1.1f);
    }
    ...
    dl->AddRect(bb.Min, bb.Max, IM_COL32(18, 18, 24, 255));

    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    if (hovered)
    {
        ImVec4 c = ColorConvertU32ToFloat4(text_col);
        c.x = ImMin(c.x * 1.15f, 1.0f);
        c.y = ImMin(c.y * 1.15f, 1.0f);
        c.z = ImMin(c.z * 1.15f, 1.0f);
        text_col = GetColorU32(c);
    }
    if (held) {
        ImVec4 c = ColorConvertU32ToFloat4(text_col);
        c.x = ImMin(c.x * 2.0f, 1.0f);
        c.y = ImMin(c.y * 2.0f, 1.0f);
        c.z = ImMin(c.z * 2.0f, 1.0f);
        text_col = GetColorU32(c);
    }


    ImVec2 text_pos = ImVec2(
        bb.Min.x + (bb.GetWidth() - label_size.x) * 0.5f,
        bb.Min.y + (bb.GetHeight() - label_size.y) * 0.5f
    );
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = avail.x - 68.0f;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float slider_height = GetFrameHeight() * 0.45f;

    ImVec2 cursor = window->DC.CursorPos;
    cursor.y += 12.0f;
    cursor.x += 20.0f;

    const ImRect frame_bb(cursor, cursor + ImVec2(w, slider_height));
    ...
    auto Lighter = [](ImU32 col, float factor) {
    ...
    float fraction = (grab_bb.GetCenter().x - frame_bb.Min.x) / frame_bb.GetWidth();
    fraction = ImClamp(fraction, 0.0f, 1.0f);
    float cut = frame_bb.Min.x + fraction * frame_bb.GetWidth();

    ImU32 left_top = IM_COL32(0xC9, 0x6F, 0x83, 255);
    ImU32 left_bottom = IM_COL32(0x88, 0x43, 0x51, 255);
    ImU32 right_top = IM_COL32(0x22, 0x22, 0x2B, 255);
    ImU32 right_bottom = IM_COL32(0x2E, 0x2E, 0x35, 255);
    ImU32 border_col = IM_COL32(0x12, 0x12, 0x18, 255);

    if (hovered) {
        left_top = Lighter(left_top, 1.2f);
        left_bottom = Lighter(left_bottom, 1.2f);
        right_top = Lighter(right_top, 1.2f);
        right_bottom = Lighter(right_bottom, 1.2f);
        border_col = Lighter(border_col, 1.2f);
    }

    if (cut > frame_bb.Min.x)
        dl->AddRectFilledMultiColor(frame_bb.Min, ImVec2(cut, frame_bb.Max.y),
            left_top, left_top, left_bottom, left_bottom);
    if (cut < frame_bb.Max.x)
        dl->AddRectFilledMultiColor(ImVec2(cut, frame_bb.Min.y), frame_bb.Max,
            right_top, right_top, right_bottom, right_bottom);

    dl->Flags |= ImDrawListFlags_AntiAliasedLines;
    ImVec2 min(roundf(frame_bb.Min.x), roundf(frame_bb.Min.y));
    ImVec2 max(roundf(frame_bb.Max.x), roundf(frame_bb.Max.y));
    dl->AddRect(min, max, border_col, 0.0f, 0, 1.0f);
    ...
    ImVec2 value_pos(grab_center.x - value_size.x * 0.5f, frame_bb.Max.y - 8.0f);

    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ImU32 outline_col = IM_COL32(0, 0, 0, 180);
    float outline_thickness = 1.0f;

    dl->AddText(Style::FontBold, Style::FontBold->FontSize, value_pos + ImVec2(-outline_thickness, 0), outline_col, value_buf);
    dl->AddText(Style::FontBold, Style::FontBold->FontSize, value_pos + ImVec2(outline_thickness, 0), outline_col, value_buf);
    dl->AddText(Style::FontBold, Style::FontBold->FontSize, value_pos + ImVec2(0, -outline_thickness), outline_col, value_buf);
    dl->AddText(Style::FontBold, Style::FontBold->FontSize, value_pos + ImVec2(0, outline_thickness), outline_col, value_buf);

    dl->AddText(Style::FontBold, Style::FontBold->FontSize, value_pos, text_col, value_buf);

    if (label_size.x > 0.0f)
    {
        ImVec2 label_pos(frame_bb.Min.x,
            frame_bb.Min.y - label_size.y - 4.0f);
        ImU32 label_col = GetColorU32(ImGuiCol_Text);
        if (hovered)
            label_col = Lighter(label_col, 1.75f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    // Slimmer input height (similar to your custom buttons)
    float input_height = is_multiline ? g.FontSize * 8.0f : g.FontSize * 1.6f; // adjust 1.2f → tighter or looser
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), input_height);
    float pad_y = is_multiline ? style.FramePadding.y
        : ImMax(0.0f, (frame_size.y - g.FontSize) * 0.5f);
...
// Render frame
    if (!is_multiline)
    {
        ...
        // ----- Custom Render Frame (same as ButtonEx style) -----
        auto Lighter = [](ImVec4 c, float f)
            {
                c.x = ImMin(c.x * f, 1.0f);
                c.y = ImMin(c.y * f, 1.0f);
        ...
        ImVec4 top = ImVec4(41 / 255.f, 41 / 255.f, 49 / 255.f, 1.0f); // #292931
        ImVec4 bottom = ImVec4(32 / 255.f, 32 / 255.f, 39 / 255.f, 1.0f); // #202027
        ...
        ImDrawList* dl = draw_window->DrawList;
        dl->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max,
            GetColorU32(top), GetColorU32(top),
            GetColorU32(bottom), GetColorU32(bottom));

        // Border
        dl->AddRect(frame_bb.Min, frame_bb.Max, IM_COL32(18, 18, 24, 255));
    }


    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline
        ? draw_window->DC.CursorPos
        : frame_bb.Min + ImVec2(style.FramePadding.x, pad_y);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    {
        float w = square_sz * 1.1f;
        float h = square_sz * 0.5f;

        float avail_width = ImGui::GetContentRegionAvail().x;
        if (avail_width > w)
        {
            ImGui::Dummy(ImVec2(avail_width - w - 9.f, 0.0f));
            ImGui::SameLine();
        }
        ...
        preview_size = ImVec2(w, h);

        int m = 28;
        ImGui::SetCursorPosX(ImGui::GetCursorPosX() - m);

        ImVec4 top = ImVec4(col[0] * 1.4f, col[1] * 1.4f, col[2] * 1.4f, col[3]);
        ImVec4 bottom = ImVec4(col[0] * 0.5f, col[1] * 0.5f, col[2] * 0.5f, col[3]);

        top.x = ImMin(top.x, 1.0f);
        top.y = ImMin(top.y, 1.0f);
        top.z = ImMin(top.z, 1.0f);
        bottom.x = ImMax(bottom.x, 0.0f);
        bottom.y = ImMax(bottom.y, 0.0f);
        bottom.z = ImMax(bottom.z, 0.0f);

        dl->Flags |= ImDrawListFlags_AntiAliasedLines;

        float offset_top = 1.0f;
        float offset_bottom = -1.0f;

        dl->AddRectFilledMultiColor(
            pos + ImVec2(-m + 0.6, offset_top),
            pos + preview_size + ImVec2(-m -0.5, offset_bottom),
            ImColor(top), ImColor(top), ImColor(bottom), ImColor(bottom)
        );

        dl->Flags &= ~ImDrawListFlags_AntiAliasedLines;

        if (ImGui::ColorButton("##ColorButton", col_v4, flags, preview_size))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                g.ColorPickerRef = ImVec4(col[0], col[1], col[2], col[3]);
                OpenPopup("picker");
                SetNextWindowPos(g.LastItemData.Rect.GetTR() + ImVec2(style.ItemSpacing.x, 0.0f));
            }
        }
        ...
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(Style::MainChildBg));
        ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(Style::BorderChild));
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                float inner_border_thickness = 1.0f;
                ImU32 inner_border_color = GetColorU32(ImVec4(Style::BorderInner));

                draw_list->AddRect(popup_pos + ImVec2(inner_border_thickness, inner_border_thickness),
                    popup_pos + popup_size - ImVec2(inner_border_thickness, inner_border_thickness),
                    inner_border_color, 0.0f, ImDrawFlags_None, inner_border_thickness);

                ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaBar;
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;

                SetNextItemWidth(square_sz * 8.0f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            ImDrawList* draw_list = window->DrawList;
            draw_list->Flags |= ImDrawListFlags_AntiAliasedLines;  // Enable anti-aliasing for lines

            // Drawing the border with the anti-aliased flag
            draw_list->AddRect(bb.Min, bb.Max, ImColor(13, 13, 19), rounding);

            // Reset the flag to avoid affecting other drawing commands
            draw_list->Flags &= ~ImDrawListFlags_AntiAliasedLines;  // Reset after use
        }
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (hovered && !selected)
        {
            ImVec4 txt = ImGui::GetStyleColorVec4(ImGuiCol_Text);
            ImVec4 lighter_txt = ImVec4(
                txt.x + (1.0f - txt.x) * 1.0f,
                txt.y + (1.0f - txt.y) * 1.0f,
                txt.z + (1.0f - txt.z) * 1.0f,
                1.0f
            );
            text_col = ImColor(lighter_txt);
        }

        if (selected)
        {
            bg_col = IM_COL32(54, 54, 64, 255); // #363640
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (selected)
        {
            bg_col = IM_COL32(0x25, 0x25, 0x2D, 255);
            ImVec4 accent = Style::Accent;
            text_col = IM_COL32(
                (int)(accent.x * 255),
                (int)(accent.y * 255),
...
bool ImGui::CustomSelectable2(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float row_h = size_arg.y > 0.0f ? size_arg.y : (label_size.y + 2.0f);
    row_h = ImMax(row_h, label_size.y + 4.0f);

    ImVec2 size(size_arg.x != 0.0f ? size_arg.x : label_size.x, row_h);
    ...
    const float pad_y = (row_h - label_size.y) * 0.5f;

    const ImVec2 text_min(min_x + pad_x, pos.y + pad_y);
    const ImVec2 text_max(min_x + size.x - pad_x, pos.y + row_h - pad_y);

    ImRect bb(min_x, pos.y, min_x + size.x, pos.y + row_h);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_ChildBg, IM_COL32(40, 40, 50, 255));
    ImGui::PushStyleColor(ImGuiCol_Border, IM_COL32(18, 18, 24, 255));
    ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 0.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(4, 4));

    BeginChild(id, frame_bb.GetSize(), ImGuiChildFlags_Borders);
